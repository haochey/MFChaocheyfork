...
module m_model
    ...
    public :: f_model_read, s_model_write, s_model_free, f_model_is_inside, & 
              ...
              f_initialize_interploate_3D, f_finalize_interploate_3D
...
contains
    ...
    subroutine f_interpolate_2D(boundary_v, boundary_edge_count, spacing, interpolated_boundary_v, total_vertices)
        ...
        !    print*, 'edge:', i, 'x1', x1 , 'y1', y1, 'x2', x2 , 'y2', y2
           ...
           if (edge_length > cell_width) then
              num_segments = 20*ceiling(edge_length/cell_width)
           else
              num_segments = 1
           end if
    ...
    subroutine f_interpolate_3D(model, spacing, interpolated_boundary_v, total_vertices)
        t_vec3, intent(in) :: spacing
        type(t_model), intent(in) :: model
        real(kind(0d0)), intent(inout), dimension(:, :) :: interpolated_boundary_v
    ...
    subroutine f_initialize_interploate_3D(model, spacing, interpolated_boundary_v)
        t_vec3, intent(in) :: spacing
        type(t_model), intent(in) :: model
        real(kind(0d0)), allocatable, intent(inout), dimension(:, :) :: interpolated_boundary_v
    ...
    subroutine f_finalize_interploate_3D(interpolated_boundary_v)
        real(kind(0d0)), allocatable, intent(inout), dimension(:, :) :: interpolated_boundary_v
